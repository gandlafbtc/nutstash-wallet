...
<div class="flex w-full h-full justify-center gap-2 flex-col text-center">
	...
	{:else}
    <div>
        <button class="link mt-5" on:click={()=>step='select'}>
            Go back
        </button>
    </div>
    <Dropzone containerClasses="bg-base-200 h-full w-full rounded-lg border border-dashed flex items-center justify-center flex-col gap-10" disableDefaultStyles={true}
    ...
    >
        ...
        <p class="text-lg font-bold">Drop the <code class="text-warning">nutstash_backup.json</code> file here, or click to select a file</p>
