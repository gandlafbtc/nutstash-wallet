...
<div class="max-w-5xl w-full mi">
	<div
		class="flex max-w-5xl w-full min-h-full bg-base-100 rounded-lg justify-between items-center flex-col gap-2 p-3"
	>
		<div class="flex flex-col gap-2 w-full items-center">
			<h1 class="font-bold text-2xl">Ecash restore wizard 🪄</h1>
			<div class="flex">
			...
			<input type="range" min="10" max="200" bind:value={countStep} class="range range-secondary">
				...
				<div class="flex gap-2 p-2 rounded-lg w-full bg-base-200 justify-between items-center">
					<button
						class="btn {isAutosearched ? 'btn-disabled' : 'btn-secondary'}"
						on:click={autoSearch}
					>
						Auto search
					</button>
			...
			{:else}
				<p class="pt-10">
					Looking for mints? Visit <a
						href="https://bitcoinmints.com/"
						target="_blank"
						rel="noopener noreferrer"
						class="link link-secondary">https://bitcoinmints.com/</a
					> to find your mints
				</p>
			{/if}
			{#each $mints as mint, i}
				<MintRestoreCard {mint} {countStep} bind:mintAutoSearch={mintSearches[i]}></MintRestoreCard>
			{/each}
		</div>
