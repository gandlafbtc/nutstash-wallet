<script lang="ts">
    ...
    constructedChunks[constructedChunks.length-1] = constructedChunks[constructedChunks.length-1].padEnd(CHUNK_SIZE+12, '=')

    let activeChunk = 0

    const qrInterval = setInterval(()=> {activeChunk=(activeChunk+1) % constructedChunks.length}, 200)
    onDestroy(
        ()=>{
