...
const createRelaysStore = () => {
    ...
    const { add, addOrUpdate, remove, clear, init: initialize, reEncrypt, reset, getBy, getAllBy, addMany } = createDefaultStoreFunctions(encryptionHelper, store);

    const addDefaultRelays = async () => {
        await addMany(DEFAULT_RELAYS.map((r) => { return { url: r, isOn: true } }))
    }
    ...
    return { add, set, subscribe, update, addOrUpdate, remove, init, reset, clear, reEncrypt, getBy, getAllBy };
}
