<script lang="ts">
    ...
    const restoreKeyset = async (cashuMint: CashuMint, ks: MintKeyset) => {
        try {
            ...
            const unspentKeysetProofs = keysetProofs.filter(p=> unspentProofStateYs.includes(hashToCurve(enc.encode(p.secret)).toHex(true)))
    ...
    <div class="flex justify-between w-full gap-2">
        ...
        {#if status === 'done'}
            ...
            {:else if status === 'error'}
             ...
             <LoaderCircle class='animate-spin h-4 w-4'></LoaderCircle>
             {:else}
