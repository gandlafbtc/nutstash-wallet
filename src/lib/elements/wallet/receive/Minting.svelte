<script lang="ts">
	import { CashuMint, type OutputAmounts } from '@cashu/cashu-ts';
	import type { Mint } from '$lib/model/mint';
	import { QRCodeImage } from 'svelte-qrcode-image';
	import { decode } from '@gandlaf21/bolt11-decode';
	import { browser } from '$app/environment';
	import { toast } from '$lib/stores/toasts';
	import { mintRequests } from '$lib/stores/mintReqs';
	...
	import CustomSplits from '$lib/elements/CustomSplits.svelte';
	...
	const mintRequest = async () => {
		try {
			if (!amount) {
				toast( 'No amount provided', 'warning', 'Could not create invoice');
				return;
			}
			if (isNaN(amount) || amount <= 0) {
				toast('Amount must be greater than 0', 'warning',  'Could not create invoice');
				return;
			}
			isComplete = false;
			isLoading = true;
