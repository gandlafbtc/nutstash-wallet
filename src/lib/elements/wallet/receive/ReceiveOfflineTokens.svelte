...
<div class="flex flex-col gap-2 w-full">
    ...
    <div class="flex gap-4 flex-col border rounded-xl p-4 relative">
        <div class="absolute right-2 top-2">
            <Button variant="ghost" size='sm' disabled={isLoading} onclick={()=> {idToDelete = tx.id 
        isOpen = true;}}>
        <Trash class='text-red-500'></Trash>
            ...
            <div class="text-sm text-muted text-ellipsis overflow-hidden w-52 break-keep">
                {tx.mintUrl}
            </div>
            {#if !showQR}
            ...
            <div class="flex flex-col gap-2">
                <Button disabled={isLoading} onclick={()=>claimOneOfflineToken(tx)}>
                    Claim
                </Button>
            </div>
    </div>
...
<Dialog.Root bind:open={isOpen}>
	<Dialog.Content>
		...
		<Dialog.Footer>
			...
			<Button variant="destructive" onclick={()=> {removeOfflineToken(); isOpen = false; }}>Yes, Delete</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
