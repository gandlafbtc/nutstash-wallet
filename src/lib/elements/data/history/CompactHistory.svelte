<script lang="ts">
    ...
    import { ArrowDownRight, ArrowUpRight, Banknote, Zap, Timer, X } from "lucide-svelte";
    import { formatDistance } from "date-fns";
    import { now } from "$lib/stores/session/time";
    import type { StoredMeltQuote, StoredMintQuote, StoredTransaction } from "$lib/db/models/types";
    let items = $derived([
        ...$transactionsStore,
        ...$mintQuotesStore,
        ...$meltQuotesStore,
    ...
    const getUrlForItem = (item: (StoredTransaction| StoredMeltQuote| StoredMintQuote)): string => {
        if (item.type ==='mint') {
            ...
            return `/#/wallet/receive/cashu/${item.id}`
        }
...
<div class="flex gap-2 w-full mt-6">
    <div class="flex gap-2 flex-col w-full">
        {#each latest as item}
            <a class="border rounded-xl p-5 w-full flex justify-between items-center" href={getUrlForItem(item)}>
                {#if item.type==='mint'}
                <div class="relative">
                    <Zap class='text-yellow-500'></Zap>
                    <div class="absolute -top-1 -left-2">
                        <ArrowDownRight class='w-4 h-4 text-green-600'></ArrowDownRight>
                    </div>
                    ...
                    {/if}
                    {#if item.state === 'UNPAID' && $now/1000 > item.expiry}
                ...
                <div class="relative">
                    <Zap class='text-yellow-500'></Zap>
                    <div class="absolute -top-1 -left-2">
                        <ArrowUpRight class='w-4 h-4 text-red-600'></ArrowUpRight>
                    </div>
                </div>
                {:else if item.type==='send'}
                <div class="relative">
        ...
        {:else}
            <button class="border rounded-xl p-5 w-full flex" onclick={()=> {
