...
<div class="relative mx-auto h-[80px] w-full max-w-[60px] overflow-hidden">
	<!-- Click area to scroll up -->

	<!-- Scrollable container -->
	<div
		bind:this={containerRef}
		class="scrollbar-hide h-full snap-y snap-proximity overflow-auto"
	...
	>
		...
		<div class="pb-[10px] pt-[10px]">
			{#each mintUnits as unit, index}
				<div
					class="flex h-[60px] snap-center items-center px-2 transition-all duration-200"
				...
				>
					<span class="select-none text-muted-foreground">{unit}</span>
				</div>
			{/each}
		</div>
	</div>
	<button
		...
		class:text-muted={selectedIndex === 0}
		onclick={selectPrev}
	>
		<ChevronUp></ChevronUp>
	</button>
	...
	<button
		disabled={selectedIndex === mintUnits.length - 1}
		class="absolute bottom-0"
		class:text-muted={selectedIndex === mintUnits.length - 1}
		onclick={selectNext}
	>
